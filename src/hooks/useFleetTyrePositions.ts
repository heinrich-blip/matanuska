      ...
      if (tyreCodes.length > 0) {
        ...
        if (tyresError) {
          ...
          tyresData.forEach(tyre => {
            // Check for dot_code directly on tyre (after migration), then fallback to inventory
            const tyreRecord = tyre as Record<string, unknown>;
            ...
            const dotCode = dotCodeFromTyre || dotCodeFromInventory;
